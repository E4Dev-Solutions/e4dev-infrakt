...
All six routers share a single `require_api_key` dependency injected at registration time in `api/main.py`. Every route is protected without per-endpoint annotations.

Input validation is handled by Pydantic schemas in `api/schemas.py`, which enforce:
- Safe name patterns (alphanumeric, dots, hyphens, underscores) on server names, app names, and database names
- Valid domain name format
- HTTPS-only Git repository URLs with SSRF prevention (blocks localhost, RFC-1918 ranges, link-local addresses)
- Port ranges (1–65535)
...
├── App.tsx                # Auth gate: shows Login or the routed app layout
├── api/client.ts          # Typed fetch wrappers; manages API key in localStorage
├── hooks/useApi.ts        # TanStack Query hooks wrapping client.ts
├── pages/
│   ├── Login.tsx          # API key entry form; validates key before storing
...
└── components/
    ├── Layout.tsx         # Top-level nav with logout button
...
`App.tsx` reads the API key from `localStorage` on mount. If no key is present, it renders `Login.tsx` instead of the main application. The login page calls `GET /api/dashboard` with the entered key; on `200 OK` it calls `setApiKey()` (which writes to `localStorage`) and transitions to the main app. On logout, `clearApiKey()` removes the key from `localStorage` and re-renders the login screen.
...
- Reads the API key from `localStorage` and sends it as `X-API-Key` on every request
...
├── api_key.txt         # API key for web dashboard authentication, chmod 0600
├── envs/
...
All API routes require an `X-API-Key` header. The key is generated by `api/auth.py` (`get_or_create_api_key()`) on first startup using `secrets.token_urlsafe(32)` (256 bits of entropy) and written to `~/.infrakt/api_key.txt` with permissions `0600`.

Incoming keys are compared with `hmac.compare_digest` (constant-time comparison) to prevent timing attacks. A missing key returns `401 Unauthorized`; an incorrect key returns `403 Forbidden`.

The `api/auth.py` module exports a `require_api_key` FastAPI dependency that is applied at the router registration level in `api/main.py` — every route in every router is protected without having to annotate individual endpoints.

CORS is configured to allow `http://localhost:5173` (Vite dev server) and `http://localhost:3000` only. infrakt is designed for local use. If you expose the API on a non-localhost interface, update the `allow_origins` list in `api/main.py` appropriately.

The React frontend stores the API key in browser `localStorage` and sends it as the `X-API-Key` header on every request. The login page (`frontend/src/pages/Login.tsx`) validates the key by making a test request to `GET /api/dashboard` before storing it.
